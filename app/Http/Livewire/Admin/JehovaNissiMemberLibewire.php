...
class JehovaNissiMemberLibewire extends Component
{
    ...
    public function render()
    {
        $members = Member::orderBy($this->sortColumn, $this->sortDirection)->whereHas('neighborhood', function ($neighborhood) {
            return $neighborhood->whereHas('sector', function ($sector) {
                return $sector->where('slug', 'jehova-nissi');
            });
        });
        ...
        return view('livewire.admin.jehova-nissi-member-libewire', ['members' => $members->paginate(10), 'neighborhoods' => $neighborhoods])
